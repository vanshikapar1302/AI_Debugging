...
LEARNING_RATE        = 0.005
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.70        # FIX 5: was 0.95 — so high that softmax
                                   # outputs almost never passed the check,
                                   # causing the bot to always say "I don't
                                   # understand". 0.70 is a realistic bar for
                                   # a small RNN trained on limited data.
...
def preprocess(text: str) -> list:
    ...
    # FIX 2: was tok.isdigit() — that kept only numeric tokens (e.g. "42")
    # and discarded every real word, so the vocabulary was always empty and
    # training crashed immediately. tok.isalpha() correctly keeps words only.
...
def _one_hot(idx: int, size: int) -> np.ndarray:
    vec = np.zeros((size, 1))
    # FIX 1: was vec[idx] = 0.0 — every one-hot vector was all zeros, so the
    # RNN received no signal at all regardless of the input word. Setting the
    # target position to 1.0 produces a proper one-hot encoding.
    vec[idx] = 1.0
    return vec
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            # FIX 3: was (1.0 + self._hs[t+1] ** 2) * d_h
            # The derivative of tanh(x) is 1 - tanh²(x), not 1 + tanh²(x).
            # The wrong sign flipped every gradient, so weight updates moved
            # in the opposite direction and loss never decreased.
...
def _softmax(x: np.ndarray) -> np.ndarray:
    # FIX 4: was np.exp(x) with no stabilisation.
    # When logits are large, np.exp(x) overflows to inf, making all
    # probabilities nan and breaking training entirely. Subtracting x.max()
    # shifts the largest value to 0 before exponentiation — mathematically
    # equivalent (the constant cancels in numerator and denominator) but
    # numerically safe.
    e_x = np.exp(x - x.max())
    return e_x / e_x.sum()
